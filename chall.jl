...
function create_key(len::Int, data::Data)
  ...
  for i in 1:len
    push!(sk, rand(1:data.q))   #sk = array of 40 randoms
    pk = powermod(data.g, sk[i], data.q) * pk % data.q #2 ^ mul(sk) % q
  end
  push!(data.sks, sk)
  push!(data.pks, pk)
...
function create_secret(len::Int, data::Data)  # reads len(flag) characters
...
function encrypt(flag::String, data::Data)
  ...
  r = rand(1:data.q) #random under q
  s = data.secret[i] ⊻ Int(flag[i]) #xor
...
function main() #len(flag) == 40
  flag::String = strip(open(f->read(f, String), "flag.txt"))  #read flag  
